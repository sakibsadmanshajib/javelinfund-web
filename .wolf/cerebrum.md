...
- **TWO separate Cloudflare workers: `javelinfund-web` (root `wrangler.toml`, Astro site) and `javelinfund-decap-oauth` (`decap-oauth-worker/wrangler.toml`, receipts API).** Originally `deploy.yml` deployed only the site, so worker changes needed a manual `cd decap-oauth-worker && npx wrangler deploy`. FIXED 2026-06-10: `deploy.yml` now has a second `npm ci` + `wrangler-action` step with `workingDirectory: decap-oauth-worker`, so BOTH deploy on push to main. (worker `npm ci` needs `decap-oauth-worker/package-lock.json` — present.)
- **Apps Script: `doPost(e)` CANNOT be run from the editor Run button** — no event arg → `e.parameter` throws `TypeError: Cannot read properties of undefined`. To grant a new OAuth scope (e.g. Drive), run a dedicated zero-arg function that touches the API (`authorizeDrive()` added to form-handler.gs). Deploying a new version does NOT trigger consent; only running code that hits the API does. After consent, redeploy the web app. (2026-06-10)
- **`appsscript.json` added to repo** (apps-script/) with explicit `oauthScopes` (spreadsheets, drive, script.external_request). NOTE: this file is only authoritative if pushed to the live project via clasp; the canonical project is edited in the Apps Script web editor, so keep them in sync manually. (2026-06-10)
- **Receipt PDF letterhead:** the real Javelin logo+wordmark banner is `word/media/image2.png` inside `Javelin_Letterhead.docx` (1600×420 RGBA). Extracted, trimmed+resized to 825×256, bundled as base64 in `decap-oauth-worker/src/lib/letterhead.js` and drawn at the top of every receipt by `renderPdf.js`. It is brand art, NOT a secret — fine to commit in the bundle. The earlier plain-text-name header was the "missing letterhead". (2026-06-10)
- **Verified end-to-end 2026-06-10:** post-fix, a receipt issues `status:Active` + downloads even when Drive archival fails; re-download of an `archived:false` receipt returns a valid `%PDF` (9656 bytes) regenerated from the Sheet row. Best-effort + Sheet-as-source-of-truth both proven in prod.
